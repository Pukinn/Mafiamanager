...
public class Soulsaver {

	private Map<String, Player> playerlist;
	
	public Soulsaver(Map<String, Player> _playerlist){
	...
	public void night(){
		if (Keys.soulsaver > 0){
			OutputToReferee.command("Die Seelenretter erwachen.");
			if (Keys.round == 1){
				OutputToReferee.note("Wer ist Seelenretter?");
				
				for (int i = 0; i < Keys.soulsaver; i++){
					setFigureConsole(4);
				}
			}
			
			OutputToReferee.command("Wer soll beschützt werden?");
			...
			while (true){
				...
				if (player.alive() > 0){
					player.protect();
					Log.timestamp();
					Log.addLine("Soulsavers are protecting '"+player.name()+"'"+"\n");
					break;
				}
				else {
			...
			OutputToReferee.command("Die Seelenretter schlafen ein.");
		}
	}
